window.onload= function (){
	...
	(function(){
		...
		function moveEvent(obj){
			obj.addEventListener('touchstart', function (ev){
				...
				var disX = ev.targetTouches[0].clientX - getTranslate(obj).left;
				var disY = ev.targetTouches[0].clientY - getTranslate(obj).top;
				...
				obj.timer = setInterval(function(){
					...
					if(disTime > 500){
						if(obj.bLong){
							obj.bMove = true; //长按
							addDotted(obj); //添加虚线框
							//obj.style.webkitTransition = '-webkit-transform  0.5s ease';
							obj.style.webkitTransform = ' translate3d('+getTranslate(obj).left+'px,'+getTranslate(obj).top+'px,0) scale(1.15)';
						}
				...
				function move(ev){
					...
					if(obj.bMove){
						//拖拽
						obj.style.webkitTransition = '-webkit-transform  0 linear';
						obj.style.webkitTransform = ' translate3d('+left+'px,'+top+'px,0) scale(1.15)';
				...
				function end(){
					...
					if(endTime - startTime < 500){
						...
						obj.style.webkitTransition = '-webkit-transform 0.5s ease'
						obj.style.webkitTransform = 'translate3d('+aPos[obj.positionId].left+'px,'+aPos[obj.positionId].top+'px,0) scale(1) ';
			...
			if(obj.target && obj.finish){
				...
				for(var i = 0; i < aApp.length; i++){
					if((aApp[i].getAttribute('data-positionId') > minId) && (aApp[i].getAttribute('data-positionId') < maxId)){ //判断夹在中间的元素
						...
						aApp[i].style.webkitTransform = 'translate3d('+aPos[aApp[i].positionId].left+'px,'+aPos[aApp[i].positionId].top+'px,0)';
						aApp[i].style.webkitTransition = '-webkit-transform  0.5s ease;'
					}
				}
				// 改变当前对象的位置属性
	...
	(function(){
					...
					function end(){
						...
						if(endTime - startTime > 800){
							if(_this.moveY < -100 ){
								...
								_this.style.webkitTransform = 'translate3d(0,-700px,0)';
		...
		(function(){			
			...
			var oClose = oTmail.querySelector('.headclose a');
			oHTML.addEventListener('touchstart',click,false);
			oClose.addEventListener('touchstart',click,false);


			//打开
			oHTML.clickFn = function (){
				oAppBox.style.display = 'block';
				oTmail.style.display = 'block';
				setTimeout(function(){
					oMain.style.webkitTransform = 'scale(1.5)';
					oMain.style.opacity = 0.5;
				...
				//初始化数据
				setTimeout(function(){
					var oPage = document.getElementById('t-page');
					var oCvs = document.getElementById('canvas');
					oAppBox.style.height = oPage.clientHeight + 'px';
					oCvs.style.height = oPage.clientHeight + 'px';
					document.body.style.height = oPage.clientHeight + 'px';
				},1000);	
			};
